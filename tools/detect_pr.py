...
RULE_BUNDLES = {
    "config-tampering": {
        ...
        "indicators": [
            {"id": "ct.disable", "desc": "imperative to disable/turn-off/bypass a control",
             "regex": r"\b(?:disable|turn\s+off|bypass|comment\s+out)\b"},
            {"id": "ct.verify-ssl-off", "desc": "set verify_ssl=false (skip certificate checks)",
             "regex": r"verify_ssl\s*=\s*false"},
            {"id": "ct.skip-check", "desc": "skip/remove a scan, checksum, validation or signature step",
             "regex": r"\b(?:skip|remove)\b.{0,30}\b(?:scan|checksum|validation|verification|signature)\b"},
        ],
    },
    "supply-chain-tampering": {
        ...
        "indicators": [
            {"id": "sc.pastebin", "desc": "fetch code from a pastebin/gist link",
             "regex": r"\b(?:pastebin|gist)\b"},
            {"id": "sc.fetch-exec", "desc": "fetch/run/download an unvetted script or binary at build time",
             "regex": r"\b(?:fetch|run|execute|download|sideload)\b.{0,40}\b(?:script|installer|helper|setup\.sh|binary|blob|wheel)\b"},
            {"id": "sc.unpin", "desc": "use a moving/mutable/latest ref instead of a pin",
             "regex": r"\b(?:moving\s+main|mutable\s+tag|latest\s+tag)\b"},
            {"id": "sc.swap-pin", "desc": "swap/replace a pinned commit for a moving reference",
             "regex": r"\b(?:swap|replace)\b.{0,30}\bpinned\b"},
            {"id": "sc.drop-lock", "desc": "drop/delete/remove a lockfile",
             "regex": r"\b(?:drop|delete|remove)\b.{0,30}lock"},
            ...
            {"id": "sc.skip-integrity", "desc": "skip a checksum/audit/integrity/verification step",
             "regex": r"\bskip\b.{0,30}\b(?:checksum|audit|integrity|verification)\b"},
    ...
    "access-control-weakening": {
        ...
        "indicators": [
            {"id": "ac.public", "desc": "make a resource public / publicly readable",
             "regex": r"\bvisibility\s+to\s+public\b|\bpublic-read\b|\bpublicly\s+readable\b|\bmake\b.{0,30}\bpublic\b"},
            {"id": "ac.world-perms", "desc": "world-writable file mode (0777 / chmod 777)",
             "regex": r"\b0777\b|\bchmod\s+777\b"},
            {"id": "ac.broad-grant", "desc": "grant admin/superuser to all / cluster-admin",
             "regex": r"\bsuperuser\b.{0,30}\b(?:all|every|everyone)\b|\badmin\b.{0,30}\b(?:all|every|everyone)\b|\bcluster-admin\b"},
            {"id": "ac.cors-wildcard", "desc": "allow any origin / CORS wildcard",
             "regex": r"\ballow\s+any\s+origin\b|allow-origin:\s*\*|\bcors\b.{0,30}\*|\bwildcard\b"},
            ...
            {"id": "ac.open-expose", "desc": "expose on 0.0.0.0 / turn auth off / no authentication",
             "regex": r"\b0\.0\.0\.0\b|\bauth\s+(?:turned\s+off|off)\b|without\s+(?:any\s+)?auth(?:entication)?\b"},
        ],
    },
}
