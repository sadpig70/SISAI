...
FRAUD_AML_BUNDLE = {
    ...
    "indicators": [
        {"id": "aml.structuring", "desc": "structuring/smurfing below the reporting threshold",
         "regex": r"\b(?:break|split|structure)\b.{0,30}\b(?:deposit|amount|transaction)s?\b.{0,30}\b(?:under|below|beneath)\b.{0,20}\b(?:threshold|reporting|10[,.]?000)\b|\bsmurf\w*\b|\bsub-?threshold\b"},
        {"id": "aml.layering", "desc": "layering through shell / front companies / nominees",
         "regex": r"\b(?:route|funnel|move|layer|cycle)\b.{0,30}\b(?:shell\s+compan\w+|front\s+compan\w+|nominee\w*)\b"},
        {"id": "aml.mule", "desc": "money mules — accounts in other people's names",
         "regex": r"\b(?:open|use)\b.{0,30}\baccounts?\b.{0,30}\b(?:other\s+people|someone\s+else|other'?s)\b|\bmoney\s+mules?\b|\brecruit\b.{0,20}\bmules?\b"},
        ...
        {"id": "aml.tbml", "desc": "trade-based ML — mis-invoicing",
         "regex": r"\bover-?invoic\w*|\bunder-?invoic\w*|\bmis-?invoic\w*|\bmis-?state\b.{0,15}\binvoice\b"},
        {"id": "aml.mixing", "desc": "crypto mixing / tumbling / washing",
         "regex": r"\b(?:run|pass|send)\b.{0,20}\bmixer\b|\b(?:through|via)\s+(?:a\s+)?mixer\b|\b(?:tumbler|coin\s+mixer)\b|\btumbl\w*\s+service\b|\bwash\b.{0,20}\b(?:crypto|coins?|funds?)\b"},
