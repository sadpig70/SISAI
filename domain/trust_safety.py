...
MODERATION_BUNDLE = {
    ...
    "indicators": [
        ...
        {"id": "ts.harassment", "desc": "targeted/repeated harassment",
         "regex": r"\brepeated(?:ly)?\b.{0,20}\bdemean\w*|\b(?:send|post)\b.{0,20}\bdemeaning\b|\bharass\w*\b.{0,20}\b(?:relentless\w*|repeatedly|continual\w*)\b"},
        {"id": "ts.self-harm", "desc": "encouraging self-harm",
         "regex": r"\bencourage\b.{0,20}\b(?:hurt|harm)\b.{0,15}\b(?:them|him|her|your)sel\w*|\bend\s+(?:their|your|his|her)\s+life\b"},
        ...
        {"id": "ts.doxxing", "desc": "doxxing — publishing private info",
         "regex": r"\bdox\w*\b|\b(?:publish|post|leak|expose|posting)\b.{0,25}\b(?:home\s+address|private\s+address|phone\s+number|personal\s+(?:info|details)|their\s+address)\b"},
        {"id": "ts.spam", "desc": "spam / scam link distribution",
         "regex": r"\bmass-?dm\b|\b(?:spam|blast|mass|flood)\b.{0,20}\b(?:phishing|scam|fraud|link|inbox\w*)\b"},
        {"id": "ts.brigading", "desc": "coordinated harassment / brigading",
         "regex": r"\b(?:coordinate|organize|rally)\b.{0,20}\b(?:brigade|raid|flood|pile-?on)\b|\bpile\s+onto?\b"},
    ],
}
