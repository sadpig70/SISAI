#!/usr/bin/env python3
"""Evidence-based verification for CA-001 (defensive detection accuracy).

Runs the CA-001 detector over a labeled benign/malicious suite and measures
...
Pass criteria (defensive posture — never miss an attack, keep FPs low):
    recall    == 1.0     (every malicious indicator flagged)
    precision >= 0.85    (few benign auth logs / prose mis-flagged)
...
from defenses.detectors.credential_attack_detector import load_rule, scan  # noqa: E402

_SAMPLES = os.path.join(_HERE, "tests", "credential_attack_samples.jsonl")
RECALL_MIN = 1.0
PRECISION_MIN = 0.85
...
def run() -> dict:
    ...
    return {
        "method": "credential-attack-suite",
        "samples": len(samples),
