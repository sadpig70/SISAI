#!/usr/bin/env python3
"""Evidence-based verification for SCH-001 (defensive detection accuracy).

Runs the SCH-001 detector over a labeled benign/risky suite and measures
...
Pass criteria (defensive posture — never miss a crypto/side-channel gap, keep FPs low):
    recall    == 1.0     (every risky sample flagged)
    precision >= 0.85    (few benign / PQC-safe samples mis-flagged)
...
from defenses.detectors.side_channel_detector import load_rule, scan  # noqa: E402

_SAMPLES = os.path.join(_HERE, "tests", "side_channel_samples.jsonl")
RECALL_MIN = 1.0
PRECISION_MIN = 0.85
