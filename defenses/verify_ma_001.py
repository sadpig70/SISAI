#!/usr/bin/env python3
"""Evidence-based verification for MA-001 (defensive detection accuracy).

Runs the MA-001 detector over a labeled benign/malicious suite and measures
...
Pass criteria (defensive posture — never miss a malware-automation indicator,
keep FPs low):
...
from defenses.detectors.malware_automation_detector import load_rule, scan  # noqa: E402

_SAMPLES = os.path.join(_HERE, "tests", "malware_automation_samples.jsonl")
RECALL_MIN = 1.0
PRECISION_MIN = 0.85
...
def run() -> dict:
    ...
    return {
        "method": "malware-automation-suite",
        "samples": len(samples),
