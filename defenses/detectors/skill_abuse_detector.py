#!/usr/bin/env python3
"""AS-001 detector — flag agent/skill ecosystem abuse in ingested manifests/text.

Pure stdlib, deterministic (no clock / network / AI / randomness). It SCANS
ingested skill/plugin/MCP manifests or READMEs and returns a verdict OBJECT.
It never executes or obeys the matched text — matched content stays data,
reinforcing the deterministic boundary in docs/SELF-DEFENSE.md (data != instruction)
and SISAI's vendored-skills + whitelist + version-pinning doctrine.

Usage (programmatic):
    from defenses.detectors.skill_abuse_detector import load_rule, scan
    rule = load_rule()
...
_DEFAULT_RULE = os.path.join(os.path.dirname(_HERE), "rules", "AS-001-skill-abuse.json")
_SEV_ORDER = {"low": 0, "medium": 1, "high": 2, "critical": 3}


def load_rule(path: str = _DEFAULT_RULE) -> dict:
    """Load the AS-001 rule JSON and pre-compile its patterns (deterministic)."""
    ...
    for pid, sev, rx in rule.get("_compiled", []):
        if rx.search(text or ""):
            ...
            if top is None or _SEV_ORDER.get(sev, 0) > _SEV_ORDER.get(top, 0):
                top = sev
    return {
        "rule_id": rule.get("rule_id", "AS-001"),
