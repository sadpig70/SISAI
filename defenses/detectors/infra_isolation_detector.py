#!/usr/bin/env python3
"""II-001 detector — flag multi-tenant infra isolation-bypass indicators.

Pure stdlib, deterministic (no clock / network / AI / randomness). It SCANS
ingested infra logs / config / audit text and returns a verdict OBJECT. It never
executes or acts on the matched text — matched content stays data, reinforcing
the deterministic boundary in docs/SELF-DEFENSE.md (data != instruction).

Usage (programmatic):
    from defenses.detectors.infra_isolation_detector import load_rule, scan
    rule = load_rule()
...
_DEFAULT_RULE = os.path.join(os.path.dirname(_HERE), "rules", "II-001-infra-isolation.json")
_SEV_ORDER = {"low": 0, "medium": 1, "high": 2, "critical": 3}


def load_rule(path: str = _DEFAULT_RULE) -> dict:
    """Load the II-001 rule JSON and pre-compile its patterns (deterministic)."""
...
def scan(text: str, rule: dict) -> dict:
    """Return a verdict for one piece of ingested infra text (advisory data, not control)."""
    matched = []
    top = None
    for pid, sev, rx in rule.get("_compiled", []):
        if rx.search(text or ""):
            ...
            if top is None or _SEV_ORDER.get(sev, 0) > _SEV_ORDER.get(top, 0):
                top = sev
    return {
        "rule_id": rule.get("rule_id", "II-001"),
