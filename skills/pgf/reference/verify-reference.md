...
| **Code Quality** | Reuse, quality, efficiency of changed code | Same (runtime review/simplify capability or direct inspection) |
...
Verify changed code using the current runtime's review/simplify capability when available. If no such tool exists, inspect the changed files directly against the quality checks below.
...
- Issues found by the runtime review/simplify capability, or by direct inspection, are collected into a `QualityReport`
...
def verify_code_quality(workplan_path: str) -> QualityReport:
    """Changed code quality verification — runtime review/simplify or direct inspection"""
    changed_files = AI_identify_changed_files(workplan_path)
    return AI_run_quality_check(changed_files)
