...
Loop Skill is the most complete expression of PGF's parser-free property: **the execution engine itself is specified in PGF**. The active AI runtime reads loop-reference.md to understand the execution algorithm, then applies it to WORKPLAN-{Name}.md.
...
loop-reference.md    →  Execution engine (itself a PGF document)
     ↓ runtime
AI runtime        →  active model/agent runtime (runtime, not a tool)
```
...
Steps performed by the active AI runtime:
