...
Discovery Engine is ClNeo's discovery engine that automatically executes the IdeaFirst 7-stage pipeline using 14 PGF design personas.
...
User: "/PGF discover" or "find me ideas"
    ↓
Load 14 personas from personas.json
    ↓
STEP 1~6: Run 14 Agents in parallel at each stage → Aggregate results → Feed into next stage
    ↓
...
```
<skill-dir>/discovery/
    personas.json           ← 14 persona profile data
...
**Options:**
- `--personas N`: Number of personas to use (default: 14, minimum: 2)
...
The 14 personas are independently defined as agent files at `{SKILL_DIR}/agents/pgf-persona-p{1~14}.md`.
Each persona file has built-in role, system prompt, and optional model/tool hints. Runtimes with subagent support pass only the task prompt plus persona file. Runtimes without subagents execute the same personas sequentially in the current session.
...
At each stage, **issue 14 Agent tool calls simultaneously in a single message** for all 14 personas.
...
...
Agent(description="IdeaFirst P13 Historical Cycle Analyst", prompt=task_prompt, agent="{SKILL_DIR}/agents/pgf-persona-p13.md")
Agent(description="IdeaFirst P14 Mechanism Designer", prompt=task_prompt, agent="{SKILL_DIR}/agents/pgf-persona-p14.md")
```

- Persona files may include model/tool hints. Treat them as advisory unless the current runtime explicitly supports those fields.
...
After each stage completes, aggregate 14 results into a single `.md` file:
...
**Output**: `.pgf/discovery/system_design.md` (14x3 = 42 ideas)
...
**Convergence Judgment**: Analyze the final selections of 14 personas:
- Majority (8+) agreement → `CONVERGED` — Selected idea is finalized
...
1. **Extract final selections from 14 personas**: Parse each persona's selected idea from final_idea.md
2. **Vote tally**: Count votes per idea
3. **Selection decision**:
   - Majority (8+) agreement → `CONVERGED` — Immediately select the consensus idea
...
[ClNeo CREATE] Phase 1 DISCOVER complete
  Selection: "{idea_name}" | consensus: CONVERGED | votes: 9/14
...
3. **No unsafe compaction between stages**: Pipeline continuity must be maintained. If compaction is required, checkpoint `.pgf/discovery/` outputs first.
...
| Situation | Response |
|-----------|----------|
| 1~6 Agents fail (timeout, etc.) | Re-run only failed personas, preserve remaining results |
| 7+ Agents fail (below majority) | **Invalidate stage** → Re-run all 14 personas |
| 8+ Agents fail (majority failure) | **Halt stage** → Report error to user, stop pipeline |
...
| Context limit reached | Save outputs up to current stage → runtime compaction/checkpoint → Resume with `--from-step` |

### Minimum Quorum Rule

Each stage's validity requires **at least 7 personas' successful results**:

- 7+ successes → Stage valid, proceed to next stage
- 6 or fewer successes → Stage invalid, re-run or halt
- Quorum still not met after re-run → Halt + report to user

```text
[Discovery Engine] ✗ STEP {N} halted
  Successful Agents: {count}/14 (below minimum quorum of 7)
...
## 10. Team-Capable Runtime Mode (Experimental)

### Overview

An alternative execution mode for runtimes that support persistent multi-agent teams or peer-to-peer subagent messaging. It enables **real-time cross-pollination** between personas.

### Standard vs Team-Capable Execution

| Aspect | Standard (Subagent) | Team-Capable Runtime |
|--------|---------------------|-------------|
| Communication | One-way (persona → main) | Peer-to-peer (persona ↔ persona), when supported |
| Cross-pollination | Stage-level (via aggregated results) | Real-time (mid-stage messaging) |
| Coordination | Main agent aggregates | Team lead or runtime coordinator orchestrates |
| Cost | Lower (parallel subagents) | Higher (persistent sessions) |
| Quality | Good diversity | Potentially deeper synthesis |

### When to Use Team-Capable Mode
...
STEP 3-4: Team-capable mode (peer-to-peer interaction enabled)
...
```
/PGF discover --teams        # Full team-capable mode
/PGF discover --teams-hybrid # Hybrid: team-capable mode for STEP 3-4 only (recommended)
```

### Implementation Note

Team-capable execution is optional. Fallback to standard subagent mode, or sequential persona simulation, if the current runtime lacks persistent team messaging or encounters errors.
